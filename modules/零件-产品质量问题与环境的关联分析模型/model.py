...
if __name__ == '__main__':
    ...
    parser.add_argument("-data_path", "--data_path", help="data path", type=str, default = './predictData.txt') #从命令行接受数据的地址，默认是同一文件夹下的workers_list.txt文件
    ...
    with open(data_path, 'r') as f:
	    lines = f.readlines()
    line = lines[0]
    data = np.array([[float(e) for e in v.split('_')] for v in line.split(' ')])  #counts * feature_dim
    feature_dim = data.shape[1]
    dict_data = {}
    for idx in range(feature_dim):
        key = 'x' + str(idx)
        dict_data[key] = data[:, idx]
	# load the data into a dataframe:
    data = pd.DataFrame(dict_data)
    row_count = sum(1 for row in data)
    labels = [i for i in range(row_count)]
    #得到因果图矩阵 type: ndarray
    ...
    for idx in range(row_count):
    	...
    	if row_[-1] == 1:
    		cause.append(idx)
    result_list = open('./result/predictResult.txt', 'w')
    result_list.write('与零件产品质量问题关联性最强的两类环境因素为'+str(cause))
    result_list.close()